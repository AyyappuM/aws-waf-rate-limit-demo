...
# ---------------------------------------------------------------------------
if [ -d "${ROOT_DIR}/03-waf/.terraform" ]; then
  echo "==> [1/3] Destroying 03-waf"
  ALB_ARN="$(cd "${ROOT_DIR}/02-ecs" && terraform output -raw alb_arn)"
  cd "${ROOT_DIR}/03-waf"
...
# ---------------------------------------------------------------------------
if [ -d "${ROOT_DIR}/02-ecs/.terraform" ]; then
  echo "==> [2/3] Destroying 02-ecs"
  VPC_ID="$(cd "${ROOT_DIR}/01-network" && terraform output -raw vpc_id)"
  PUBLIC_SUBNET_IDS="$(cd "${ROOT_DIR}/01-network" && terraform output -json public_subnet_ids)"
  ALB_SG_ID="$(cd "${ROOT_DIR}/01-network" && terraform output -raw alb_security_group_id)"
  ECS_SG_ID="$(cd "${ROOT_DIR}/01-network" && terraform output -raw ecs_tasks_security_group_id)"
...
# Phase 01: network
# ---------------------------------------------------------------------------
if [ -d "${ROOT_DIR}/01-network/.terraform" ]; then
